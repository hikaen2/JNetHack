...
#ifdef JNETHACK
    {"ペットへの攻撃", "行う", "行なわない", &flags.safe_dog, 1},
    {"友好的な怪物への攻撃", "確認する", "確認しない", &flags.confirm},
#ifdef TEXTCOLOR
    {"ペット", "赤枠で囲む", "囲まない", &iflags.hilite_pet},
#endif
#ifdef RADAR
    {"レーダー", "表示する", "表示しない", &flags.radar},
#endif
    {NULL,},
    {"経験値", "表示する", "表示しない", &flags.showexp},
#ifdef SCORE_ON_BOTL
    {"スコア", "表示する", "表示しない", &flags.showscore},
#endif
    {"歩数", "表示する", "表示しない", &flags.time},
    {NULL,},
    {"自動でアイテムを", "拾う", "拾わない", &flags.pickup},
    {NULL,},
    {"開始時のイントロダクション", "表示する", "表示しない", &flags.legacy},
#ifdef NEWS
    {"開始時のニュース", "表示する", "表示しない", &iflags.news},
#endif
#ifdef MAIL
    {"メールデーモン", "登場する", "登場しない", &flags.biff},
#endif
    {NULL,},
    {"スペースキーで休む", "休む", "休まない", &flags.rest_on_space},
    {"ゲーム中のメッセージ", "詳しく行う", "行なわない", &flags.verbose},
#ifdef NEWBIE
    {"初心者へのヒント", "行う", "行なわない", &flags.newbie},
#endif
    {NULL,},
/*
    {"終了時の情報", "表示する", "表示しない", &flags.end_disclose},
*/
    {"終了時の墓石", "表示する", "表示しない", &flags.tombstone},
#ifdef NH_EXTENSION_REPORT
    {"スコアの報告", "報告する", "報告しない", &flags.reportscore},
#endif
    {NULL,},
    {"アイテム目録その1", "できるだけ同じ文字にする", "しない", &flags.invlet_constant},
    {"アイテム目録その2", "似たような種類をまとめる", "まとめない", &flags.sortpack},
#else
...
/*  gtk_container_border_width(GTK_CONTAINER(frame), NH_PAD);*/
...
#ifdef JNETHACK
  label = nh_gtk_new_and_pack(gtk_label_new("名前:"), hbox, "", FALSE, FALSE, NH_PAD);
#else
  ...
  radio_m = nh_gtk_new_and_pack(
      gtk_radio_button_new_with_label(female_group, "男"),
  ...
  radio_f = nh_gtk_new_and_pack(
      gtk_radio_button_new_with_label(female_group, "女"),
      hbox2, "", FALSE, FALSE, NH_PAD);
#else
  radio_f = nh_gtk_new_and_pack(
...
nh_option_new()
{
  ...
  button1 = nh_gtk_new_and_pack(
      gtk_button_new_with_label("完了"), hbox, "",
      FALSE, FALSE, NH_PAD);
  button3 = nh_gtk_new_and_pack(
      gtk_button_new_with_label("詳細設定"), hbox, "",
      FALSE, FALSE, NH_PAD);
  button2 = nh_gtk_new_and_pack(
      gtk_button_new_with_label("キャンセル"), hbox, "",
      FALSE, FALSE, NH_PAD);
#else
  button1 = nh_gtk_new_and_pack(
