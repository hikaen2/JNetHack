/*
  $Id: gtkmap.c,v 1.3 1999/12/01 03:51:06 issei Exp issei $
 */
/*
...
static TileTab	Big3DTile = {
    "Big 3D tiles",
    "floor.xpm",
    960, 3200,		/* tile map width height */
    48, 64,		/* unit_width, unit_height */
    16, 32,		/* 3d_ofset, 3d_ofsety */
    True,
    True,
};
#endif
...
static TileTab BigTile = {
    "Big tiles",
    "t32-1024.xpm",
    1024, 1024,		/* tile map width height */
    32, 32,		/* width, height */
    0, 0,
    True,
    False,
...
static TileTab SmallTile = {
    "Small tiles",
    "x11tiles",		/* tile map width height */
    16, 16000,		/* width, height */
    16, 16,
    0, 0,
    False,
    False,
...
#ifdef BIGTILE
static TileTab *Tile = &BigTile;
static int	map_visual = 2;
#else
static TileTab *Tile = &SmallTile;
static int	map_visual = 1;
#endif

static char	*NH_TILE_FILE;
static int	NH_TILE_PER_COL;
...
#define FLOOR_SRC_X (((345 + 394 + 12) % NH_TILE_PER_COL) * NH_TILE_WIDTH)
#define FLOOR_SRC_Y (((345 + 394 + 12) / NH_TILE_PER_COL) * NH_TILE_HEIGHT)
...
fix_tile(int tile)
{
    ...
    if(map_visual == 0 || map_visual == 1)
	return tile;

#ifndef JNETHACK
    if(orgtile >= glyph2tile[PM_HEALER])
	++tile;
    if(orgtile >= glyph2tile[PM_HIPPOCRATES])
	++tile;
    if(orgtile >= glyph2tile[PM_CYCLOPS])
	++tile;
    if(orgtile >= glyph2tile[PM_ATTENDANT])
	++tile;
    if(orgtile >= glyph2tile[GLYPH_OBJ_OFF + HAWAIIAN_SHIRT])
	++tile;
#endif
...
{
    GTK_curs(NHW_MAP, event->x / 32, event->y / 32);
...
nh_map_init()
{
    ...
    NH_TILE_PER_COL =		(Tile->tilemap_width / Tile->unit_width);
...
/*
  ...
  GTK nethack automaticaly set font path to HACKDIR
  if server and client are running at same host.
  */
    if(shmflg){
	char **path;
	char **new_path;

	path = XGetFontPath(display, &n);
	for(i=0 ; i<n ; ++i)
	    if(!strncmp(path[i], HACKDIR, strlen(HACKDIR)))
		goto NextStep;

	new_path = (char **)malloc(sizeof(char *) * (n + 1));
	for(i=0 ; i<n ; ++i)
	    new_path[i] = path[i];

	new_path[n] = HACKDIR;

	XSetFontPath(display, new_path, n+1);
    NextStep:
    }
#endif
...
#ifdef JNETHACK
    gtk_window_set_title(GTK_WINDOW(radar), "JNetHack Radar");
#else
    gtk_window_set_title(GTK_WINDOW(radar), "NetHack Radar");
#endif
...
#ifdef TEXTCOLOR	
    gdk_draw_rectangle(
	map_pixmap, map_color_gc[MAP_BACKGROUND],
...
nh_map_print_glyph_tmp(struct tilemap *tmap, int ofsx, int ofsy, int flag)
{
    ...
    src_x = (tile % NH_TILE_PER_COL) * c_width;
    src_y = (tile / NH_TILE_PER_COL) * c_height;
    
    bgsrc_x = (bgtile % NH_TILE_PER_COL) * c_width;
    bgsrc_y = (bgtile / NH_TILE_PER_COL) * c_height;
