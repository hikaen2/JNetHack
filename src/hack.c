...
/*JP	    pline("You're too small to push that %s.", xname(otmp));*/
	...
	if (isok(rx,ry) && !IS_ROCK(levl[rx][ry].typ) &&
	    ...
	    if (In_sokoban(&u.uz) && u.dx && u.dy) {
	    	if (Blind) feel_location(sx,sy);
/*JP	    	pline("%s won't roll diagonally on this %s.",
...
/*JP		    pline("There's %s on the other side.", mon_nam(mtmp));*/
		    pline("反対側に%sがいる．", mon_nam(mtmp));
		else {
		    if (Blind) feel_location(sx, sy);
/*JP		    You_hear("a monster behind %s.", the(xname(otmp)));*/
		    pline("%sの背後に怪物の気配がする．", the(xname(otmp)));
		    map_invisible(rx, ry);
		}
		if (flags.verbose)
/*JP		    pline("Perhaps that's why %s cannot move it.",*/
		    pline("たぶんこれが，%sが動かせない理由だ．",
#ifdef STEED
...
#ifdef STEED
		    if (u.usteed)
/*JP			pline("%s pushes %s and suddenly it disappears!",
...
/*JP		    pline("With %s effort you move %s.",
...
/*JP		    pline("%s moves %s.", Monnam(u.usteed), the(xname(otmp)));*/
		    pline("%sは%sを動かした．", Monnam(u.usteed), the(xname(otmp)));
#endif
		lastmovetime = moves;
	    }
...
#ifdef STEED
	  if (u.usteed)
/*JP	    pline("%s tries to move %s, but cannot.",
		  	Monnam(u.usteed), the(xname(otmp)));
*/
...
/*JP			You("aren't skilled enough to %s %s from %s.",
				flags.pickup ? "pick up" : "push aside",
				the(xname(otmp)), mon_nam(u.usteed));
*/
			You("%sに%sを%sさせるほど技量がない．",
				mon_nam(u.usteed),
				the(xname(otmp)), 
				flags.pickup ? "拾わ" : "押");
		else
#endif
/*JP		  pline("However, you can easily %s.",
...
/*JP		    You("move right into %s.", mon_nam(mtmp));*/
...
/*JP		You("attack %s.", Underwater ? "empty water" : "thin air");*/
		You("%sを攻撃した", Underwater ? "何もない水中" : "何もない空間");
...
/*JP		pline("%s won't move!", Monnam(u.usteed));*/
		pline("%sは動かない！", Monnam(u.usteed));
		nomul(0);
		return;
	} else
#endif
...
/*JP			You("try to ooze under the door, but can't squeeze your possessions through.");*/
...
/*JP		    You("enter an opulent throne room!");*/
		    ...
		    break;
		case LEPREHALL:
/*JP		    You("enter a leprechaun hall!");*/
		    You("レプラコーンホールに入った！");
		    break;
		case MORGUE:
		    if(midnight()) {
/*JP			const char *run = locomotion(youmonst.data, "Run");*/
			const char *run = locomotion2(youmonst.data, "逃げろ");
...
/*JP		    You("enter a giant beehive!");*/
		    ...
		    break;
		case COCKNEST:
/*JP		    You("enter a disgusting nest!");*/
		    You("むっとする臭いのする鳥の巣に入った！");
		    break;
		case ANTHOLE:
/*JP		    You("enter an anthole!");*/
		    You("アリの巣に入った！");
		    break;
		case BARRACKS:
...
/*JP			verbalize("%s, %s, welcome to Delphi!", Hello(), plname);*/
...
/*JP		    You("pick up %s tongue.", s_suffix(mon_nam(u.ustuck)));
...
/*JP		You_cant("even see the bottom, let alone pick up %s.",
				something);
*/
...
/*JP		You("die...");*/
		...
		if (Role_if(PM_WIZARD) || Race_if(PM_ELF) || Role_if(PM_VALKYRIE)) {
			if (u.uhp == 1)
/*JP				pline("%s is about to die.",*/
				pline("%sは死にかけている．",
				      jtrns_mon(
					  (Role_if(PM_WIZARD) || Role_if(PM_VALKYRIE)) ?
					  urole.name.m : "Elf", flags.female));
