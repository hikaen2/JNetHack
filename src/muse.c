...
/*JP			You_hear("a horn being played.");*/
	...
	case MUSE_SCR_EARTH:
		...
		mreadmsg(mtmp, otmp);
	    	/* Identify the scroll */
		if (canspotmon(mtmp)) {
/*JP		    pline_The("%s rumbles %s %s!", ceiling(mtmp->mx, mtmp->my),
...
/*JP		    pline_The("%s rumbles in the middle of nowhere!",
			ceiling(mtmp->mx, mtmp->my));
*/
...
/*JP				    pline("%s is hit by %s!", Monnam(mtmp2),
	    	    	    			doname(otmp2));
*/
				    pline("%sは%sに命中した！",
					  doname(otmp2),
				...
				if (helmet) {
					...
					if (canspotmon(mtmp2))
/*JP					    pline("Fortunately, %s is wearing a hard helmet.", mon_nam(mtmp2));*/
...
/*JP					    You_hear("a clanging sound.");*/
					...
					if (canspotmon(mtmp2))
						...
						him[pronoun_gender(mtmp2)]);
	    	    	    	...
	    	    	    	mtmp2->mhp -= mdmg;
	    	    	    	if (mtmp2->mhp <= 0) {
/*JP				    pline("%s is killed.", Monnam(mtmp2));*/
...
/*JP			pline("Suddenly you cannot see %s.", nambuf);*/
	...
	case MUSE_POT_SPEED:
		...
		if (vismon)
/*JP		    pline("%s is suddenly moving faster.", Monnam(mtmp));*/
		    pline("%sは突然速く動けるようになった．", Monnam(mtmp));
...
/*JP		if (vismon) pline("%s suddenly mutates!", Monnam(mtmp));*/
	...
	case MUSE_POLY_TRAP:
		if (vismon)
/*JP		    pline("%s deliberately %s onto a polymorph trap!",
			Monnam(mtmp),
			makeplural(locomotion(mtmp->data, "jump")));
*/
		    pline("%sはわざと変化の罠に%s！",
			  Monnam(mtmp),
			  jconj(locomotion(mtmp->data, "飛びこむ"), "た"));
	...
	case MUSE_BULLWHIP:
		...
		if (uwep && !rn2(5)) {
/*JP		    const char *The_whip = vismon ? "The bullwhip" : "A whip";*/
...
/*JP			pline("%s fails to wrap around %s.",
			      The_whip, the_weapon);
*/
...
/*JP		    pline("%s wraps around %s you're wielding!",
			  The_whip, the_weapon);
*/
		    pline("%sはあなたの装備している%sにからみついた！",
			  The_whip, the_weapon);
		    if (obj->cursed) {
/*JP			pline("%s is welded to your %s%c",
			      (obj->quan == 1L) ? "It" : "They",
			      hand, !obj->bknown ? '!' : '.');
*/
		    ...
		    if (!where_to) {
/*JP			pline("The whip slips free.");  / * not `The_whip' */
...
/*JP			    pline("%s yanks %s from your %s!", Monnam(mtmp),
				  the_weapon, hand);
*/
...
/*JP			    pline("%s yanks %s to the %s!", Monnam(mtmp),
...
/*JP			    pline("%s snatches %s!", Monnam(mtmp),
				  the_weapon);
*/
...
/*JP		pline(str, s_suffix(mon_nam(mon)), "amulet");*/
	    ...
	    if (str)
/*JP		pline(str, s_suffix(mon_nam(mon)), "armor");*/
...
/*JP	    	pline(fmt, str, "shield");*/
	    ...
	    if (fmt && str)
/*JP	    	pline(fmt, str, "weapon");*/
	    	pline(fmt, str, "武器");
	    return TRUE;
	} else if (EReflecting & W_AMUL) {
	    if (fmt && str) {
/*JP	    	pline(fmt, str, "medallion");*/
	    	pline(fmt, str, "メダリオン");
	    	makeknown(AMULET_OF_REFLECTION);
	    }
	    return TRUE;
	    ...
	    if (fmt && str)
/*JP	    	pline(fmt, str, "armor");*/
	    	pline(fmt, str, "鎧");
	    return TRUE;
	} else if (youmonst.data == &mons[PM_SILVER_DRAGON]) {
	    if (fmt && str)
/*JP	    	pline(fmt, str, "scales");*/
	    	pline(fmt, str, "鱗");
	    return TRUE;
	}
	return FALSE;
...
/*JP		    You("hear %s.", (obj->otyp == POT_ACID) ? "drinking" : "chewing");*/
		    You("%sという音を聞いた．", 
...
/*JP			pline("%s seems limber!", Monnam(mon));*/
