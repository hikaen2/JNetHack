...
#define EPITAPHS 25
	switch (inscription % EPITAPHS) {
		...
		pline("It is blank...");
#endif
		case 0: epitaph = "注意 --墓の中に価値あるものは＊一つも＊ない．";
		break;
		case 1: epitaph = "1994-1995 今迄でもっとも長生きしたハッカー．";
		break;
		case 2: epitaph = "名もしらぬハッカーの墓．";
		break;
		case 3:
		case 4:
		case 5: epitaph = "ここに眠る";
		break;
		case 6: epitaph = "安らかに眠れ．";
		break;
		case 7: epitaph = "ここに眠っているのは誰かは知らない．しかし我々は埋葬した．";
		break;
		case 8: epitaph = "Sparky -- すばらしい犬だった．";
		break;
		case 9: epitaph = "送電用の第三のレールに気をつけよ．";
		break;
		case 10: epitaph = "Made in Taiwan.";
		break;
		case 11: epitaph = "オレトモダチ，オレイイヤツ，オレ死んだ，オレ食料．";
		break;
		case 12: epitaph = "肉汁，肉汁，肉汁";
		break;
		case 13: epitaph = "下を見ろ！";
		break;
		case 14: epitaph = "掘らないでください．とても幸せなんです．-- 居住者";
		break;
		case 15: epitaph = "郵便屋さん．電報は下記の住所に転送してください．ゲヘナ市アスモデウス要塞5番町．";
		break;
		case 16: epitaph = "沼田一成ここに死す．";
		break;
		case 17: epitaph = "マリーは子羊を飼っていた．マリーがここで困ったとき，子羊がまっさきに来た";
		break;
		case 18: epitaph = "気をつけよ！もしくは同じことが起きる．-- 使用者";
		break;
		case 19: epitaph = "もうすぐ仲間になれる！！ -- イェンダーの魔法使い";
		break;
		case 20: epitaph = "気をつけよ！この墓は有毒な廃棄物が含まれている．";
		break;
		case 21: epitaph = "立ちされ！";
		break;
		case 22: epitaph = "ベルに助けられた．";
		break;
		case 23:
		case 24:
		default:
...
/*JP		    pline_The("scroll erupts in a tower of flame!");*/
	...
	case SCR_EARTH:
		...
		!Is_rogue_level(&u.uz) && 
	    	...
	    	/* Identify the scroll */
/*JP	    	pline_The("%s rumbles %s you!", ceiling(u.ux,u.uy),
	    	...
	    	if (!sobj->cursed) for (x = u.ux-1; x <= u.ux+1; x++) {
				...
				if (cansee(mtmp->mx, mtmp->my)) {
/*JP				    pline("%s is hit by %s!", Monnam(mtmp),
	    	    	    			doname(otmp2));
*/
				    pline("%sが%sに命中した！",
				...
				if (helmet) {
					...
					if (canspotmon(mtmp))
/*JP					    pline("Fortunately, %s is wearing a hard helmet.", mon_nam(mtmp));*/
					    pline("幸運にも%sは固いヘルメットを身につけている．", mon_nam(mtmp));
					else if (flags.soundok)
/*JP					    You_hear("a clanging sound.");*/
					    You("ガランガランという音を聞いた．");
					if (mdmg > 2) mdmg = 2;
				    } else {
					if (canspotmon(mtmp))
/*JP					    pline("%s's %s does not protect %s.",
...
/*JP	    	    	    if (!flooreffects(otmp2, x, y, "fall")) {*/
...
/*JP			You("are hit by %s!", doname(otmp2));*/
...
/*JP				Your("%s does not protect you.",
						xname(uarmh));
*/
				Your("%sでは守れない．",
						xname(uarmh));
...
/*JP		    if (!flooreffects(otmp2, u.ux, u.uy, "fall")) {*/
		    if (!flooreffects(otmp2, u.ux, u.uy, "落ちる")) {
			place_object(otmp2, u.ux, u.uy);
			stackobj(otmp2);
			newsym(u.ux, u.uy);
		    }
/*JP		    if (dmg) losehp(dmg, "scroll of earth", KILLED_BY_AN);*/
...
/*JP		pline("Where ?");*/
		...
		cc.x = u.ux;
		cc.y = u.uy;
/*JP		if (getpos(&cc, TRUE, "the desired position") < 0) / * force valid */
		if (getpos(&cc, TRUE, "狙う場所") < 0) /* force valid */
		    return 0;	/* abort */
...
/*JP			    pline("Wiped out all %s.", nam);*/
			    pline("%sを全て排除した．", jtrns_mon(nam, -1));
...
/*JP				    You_feel("dead inside.");*/
			    ...
			    if (!gameover)
/*JP				pline("All %s are already nonexistent.", nam);*/
				pline("%sは既にいない．", jtrns_mon(nam, -1));
			} else if (!gameover) {
...
*/
				You("%sを虐殺できない．",
				    jtrns_mon((uniq || named) ? mons[i].mname : nam, -1));
			    }
			}
		    }
		}
...
/*JP			pline("Such creatures %s exist in this world.",
	...
	/* fixme: unconditional "caverns" will be silly in some circumstances */
			    if(flags.verbose)
/*JP			pline("A thunderous voice booms through the caverns:");
...
/*JP			killer = "scroll of genocide";*/
	...
	/* KMH -- Unchanging prevents rehumanization */
/*JP		if (Upolyd && ptr != youmonst.data) You_feel("dead inside.");*/
		if (Upolyd && ptr != youmonst.data) You("魂が死んだような気がした．");
		else
			done(GENOCIDED);
...
/*JP		pline("Sent in some %s.", makeplural(buf));*/
	        pline("%sが送られてきた．", jtrns_mon(buf, -1));
	    else
		pline(nothing_happens);
	}
}
