...
/*JP	    verbalize("%s, %s!  Welcome%s to %s %s!",
		      Hello(), plname,
		      eshkp->visitct++ ? " again" : "",
		      s_suffix(shkname(shkp)),
		      shtypes[rt - SHOPBASE].name);
...
/*JP		pline("Pay whom?");*/
		...
		cc.x = u.ux;
		cc.y = u.uy;
		if (getpos(&cc, TRUE, "支払いたいと思う相手") < 0)
...
*/
		pline("%sは%sあなたの死体を見て%s%s．", Monnam(shkp),
		     (shkp->msleeping || !shkp->mcanmove) ? "目をさますと" : "",
		     !rn2(2) ? "首を振り，" : "",
...
/*JP		if (!is_human(youmonst.data)) Strcat(buf, " creature");*/
...
/*JP			    verbalize("%s, %s!  I was looking for %s.",
				    Hello(), plname, eshkp->customer);
*/
			    verbalize("%s！わたしは%sを探しています．",
				    Hello(TRUE), eshkp->customer);
				    eshkp->following = 0;
			    return(0);
			}
			if(moves > followmsg+4) {
/*JP			    verbalize("%s, %s!  Didn't you forget to pay?",
				    Hello(), plname);
*/
...
/*JP	    You_feel("like a common thief.");*/
	    You("泥棒同然な気がした．");
	    adjalign(-sgn(u.ualign.type));
	}
	return;
...
*/
		    verbalize("%s！私は%sを探しています．",
			    Hello(TRUE), eshk->customer);
		    eshk->following = 0;
		} else {
/*JP		    verbalize("%s %s!  Didn't you forget to pay?", Hello(), plname);*/
		    verbalize("%s！支払いを忘れていませんか？", 
			      Hello(TRUE));
		}
	} else if (eshk->billct) {
