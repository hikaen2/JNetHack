...
int what;		/* should be a long */
{
	...
	if (notake(youmonst.data)) {
	    if (!autopickup)
/*JP		You("are physically incapable of picking anything up.");*/
...
/*JP		pline("There are %s objects here.",
		      (ct <= 10) ? "several" : "many");
*/
		pline("ここには%sものがある．",
		      (ct <= 10) ? "いくつか" : "沢山の");
...
/*JP	You("cannot get your %s around this %s.",
...
*/
		pline("%sには%ld個の金塊がある, しかしこれ以上持てない．",
		      where,
		      obj->quan);
		return 0;
	    } else if (gold_capacity < count) {
/*JP		You("can only %s %s of the %ld gold pieces lying %s.",
...
/*JP			pline("Touching %s corpse is a fatal mistake.",
					an(mons[obj->corpsenm].mname));
*/
			pline("%sの死体に触れるのは致命的な間違いだ．",
					jtrns_mon(mons[obj->corpsenm].mname, -1));
/*JP			Sprintf(kbuf, "%s corpse", an(mons[obj->corpsenm].mname));*/
			Sprintf(kbuf, "%sの死体に触れて", jtrns_mon(mons[obj->corpsenm].mname, -1));
			instapetrify(kbuf);
		    return -1;
		}
	    } else if (is_rider(&mons[obj->corpsenm])) {
/*JP		pline("At your %s, the corpse suddenly moves...",
			telekinesis ? "attempted acquisition" :
			"touch");
*/
...
/*JP			You("aren't skilled enough to reach from %s.", mon_nam(u.usteed));*/
			You("まだ十分に技量を積んでいないので，%sから届かない", mon_nam(u.usteed));
		else
#endif
...
/*JP		You("cannot loot things that are deep in the %s.",
...
/*JP		    Sprintf(qbuf, "There is %s here, loot it?", doname(cobj));*/
		    Sprintf(qbuf, "ここには%sがある．開けますか？", doname(cobj));
		    c = ynq(qbuf);
...
/*JP		You("need to dig up a grave in order to properly loot it...");*/
		You("墓泥棒をするには掘らなくては．．．");
	    } else {
/*P		You("don't find anything here to loot.");*/
		pline("ここには開けるものがない．");
	    }
	}
    return (timepassed);
...
/*JP		verbalize("You sneaky cad! Get out of here with that pick!");*/
...
/*JP				      "take out", current_container->cobj,*/
