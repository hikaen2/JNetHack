...
/*JP	const char *zap_type_text = "spell";*/
...
/*JP		    pline("%s turns transparent!", nambuf);*/
...
/*JP			pline("%s turns to flesh!", name);*/
			pline("%sの石化が解けた！", name);
		} else
		    wake = FALSE;
		break;
	case SPE_DRAIN_LIFE:
		...
		if (resists_drli(mtmp))
		    ...
		    if (mtmp->mhp <= 0 || mtmp->mhpmax <= 0 || mtmp->m_lev <= 0)
		    	...
		    	mtmp->m_lev--;
		    	if (canseemon(mtmp))
/*JP		    	    pline("%s suddenly seems weaker!", Monnam(mtmp));*/
		    	    pline("%sは弱くなったような気がした！", Monnam(mtmp));
		    }
		}
		break;
	default:
...
/*JP				pline_The("figurine animates!");*/
				pline("人形は動いた！");
			    break;
			}
			goto makecorpse;
...
/*JP			    Norep("You smell the odor of meat.");*/
			    Norep("肉の悪臭が漂った．");
			else
/*JP			    Norep("You smell a delicious smell.");*/
			    Norep("すばらしい香りだ！");
			break;
		    default:
			res = 0;
...
/*JP		        You_feel("rather itchy under your %s.", xname(uarmc));*/
...
/*JP		pline("Blood drips on your %s.", body_part(FACE));*/
		pline("血があなたの%sへ落ちてきた．", body_part(FACE));
...
/*JP		    pline("Blood pools at your %s.",
...
/*JP			    hit(fltxt, mon, ".");*/
			    hit(fltxt, mon, "．");
/*JP			    pline("%s absorbs the deadly %s!", Monnam(mon),
...
/*JP		pline("%s hits you!", The(fltxt));*/
		...
		if (Reflecting) {
		    if (!Blind) {
/*JP		    	(void) ureflects("But %s reflects from your %s!", "it");*/
