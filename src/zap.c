...
/*JP	"magic missile",	/* Wands must be 0-9 *
	"bolt of fire",
	"bolt of cold",
	"sleep ray",
	"death ray",
...
/*JP	"magic missile",	/* Spell equivalents must be 10-19 *
	"fireball",
	"cone of cold",
	"sleep ray",
	"finger of death",
...
/*JP	"blast of missiles",	/* Dragon breath equivalents 20-29*
	"blast of fire",
	"blast of frost",
	"blast of sleep gas",
	...
	"blast of poison gas",
	"blast of acid",*/
	"魔法の矢の風",	/* Dragon breath equivalents 20-29*/
	"火の風",
	"氷の風",
	"睡眠ガスの風",
	"分解の風",
...
/*JP		    pline("%s resists your magic!", Monnam(mtmp));*/
		    pline("%sはあなたの魔法を防いだ！", Monnam(mtmp));
		    wake = TRUE;
		    break;
		}
		wake = TRUE;
...
/*JP				if (Blind) pline("Its mouth opens!");
	...
	case SPE_EXTRA_HEALING:
		...
		if (canseemon(mtmp))
/*JP		    pline(otyp == SPE_HEALING ? "%s begins to look better." :
			  "%s looks much better.", Monnam(mtmp));*/
		    pline(otyp == SPE_HEALING ? "%sは元気になった．" :
			  "%sはとても元気になたように見える．", Monnam(mtmp));
...
/*JP		    Norep("You cancel an unpaid object, you pay for it!");*/
		    Norep("あなたは未払のものを無力化してしまったので，支払いをした！");
...
/*JP		    Norep("You cancel it, you pay for it!");*/
		    Norep("あなたはそれを無力化してしまったので，支払いをした！");
		    bill_dummy_object(obj);
		} else
...
/*JP	    pline("Some %sobjects meld, and %s arises from the pile!",*/
	    pline("いくつかの%s物体は溶け，その山から%sが現われた！",
...
/*JP			You("imitate a popsicle!");*/
		...
		case SPE_MAGIC_MISSILE:
		    if(Antimagic) {
			shieldeff(u.ux, u.uy);
/*JP			pline("The missiles bounce!");*/
			pline("魔法の矢ははねかえった");
		    } else {
			damage = d(4,6);
/*JP			pline("Idiot!  You've shot yourself!");*/
			pline("何やってんだ！あなたは自分自身を打った！");
		    }
		    break;
		case WAN_POLYMORPH:
#ifdef POLYSELF
...
/*JP			 "Far out, man!  You can see right through yourself!" :
		...
		case WAN_SPEED_MONSTER:
		    if (!(Fast & INTRINSIC)) {
/*JP			You("seem to be moving faster.");*/
			pline("速く歩けるようになった気がした．");
...
/*JP			pline("The sleep ray hits you!");*/
			pline("睡眠光線があなたに命中した！");
			nomul(-rn2(50));
			u.usleep = 1;
...
/*JP			You("seem to be moving slower.");*/
			You("動きが鈍くなったようだ．");
...
#ifdef POLYSELF
		    if (is_undead(uasmon)) {
/*JP			You("feel frightened and %sstunned.",
			     Stunned ? "even more " : "");*/
			You("恐怖し%sくらくらした．",
			     Stunned ? "さらに" : "");
		...
		case SPE_EXTRA_HEALING:
		    healup(obj->otyp == SPE_HEALING ? rnd(8) : d(2,8)+2,
			   0, FALSE, FALSE);
/*JP		    You(obj->otyp == SPE_HEALING ? "begin to feel better." :
			"feel a fair bit better.");*/
		    You(obj->otyp == SPE_HEALING ? "気分がよくなった．" :
			"ほんのちょっと気分がよくなった．");
		    break;
		case WAN_DIGGING:
		case SPE_DIG:
...
register struct	obj	*obj;
{
	...
	if(objects[obj->otyp].oc_dir == IMMEDIATE) {
	    ...
	    if (obj_zapped)
/*JP		You("feel shuddering vibrations.");*/
		You("ぞっとする振動を感じた．"); 
...
/*JP					    You("pierce %s stomach wall!",*/
			...
			if(u.dz) {
				...
				if(u.dz < 0 || On_stairs(u.ux, u.uy)) {
					...
					pline(
/*JP			"The beam bounces off the %s and hits the ceiling.",*/
			"光線は%sで反射し天井に命中した．",
					      (u.ux == xdnladder ||
					       u.ux == xupladder) ?
/*JP					      "ladder" : "stairs");*/
					      "はしご" : "階段");
/*JP				    You("loosen a rock from the ceiling.");*/
				    pline("天井の岩がガタガタしている．");
/*JP				    pline("It falls on your %s!",*/
				    pline("それはあなたの%sに落ちた！",
					  body_part(HEAD));
/*JP				    losehp(1, "falling rock", KILLED_BY_AN);*/
				    losehp(1, "落石で", KILLED_BY_AN);
...
/*JP					pline("The wall glows then fades.");*/
					pline("壁は一瞬輝いた．");
				    break;
				}
				if(room->typ == STONE) {
...
/*JP					pline("The rock glows then fades.");*/
					pline("岩は一瞬輝いた．");
				    break;
				}
...
/*JP					"boomerang");*/
				...
				tmp_at(DISP_END, 0);
/*JP				pline("You skillfully catch the boomerang.");*/
				You("上手にブーメランをつかまえた．");
				return(&youmonst);
			}
		}
...
/*JP			    pline("%s disintegrates.", Monnam(mon));*/
			    pline("%sはこなごなになった．", Monnam(mon));
			    if(Blind)
/*JP		       You("hear the fragments of %s body reassembling!",*/
		       You("%sの体の小片が再結合するのを聞いた！",
			         s_suffix(mon_nam(mon)));
			    else
/*JP		       pline("%s body reintegrates before your %s!",*/
		       pline("%sの体はあなたの%sの前で再結合した！",
			         s_suffix(Monnam(mon)),
			         makeplural(body_part(EYE)));
/*JP		            pline("%s resurrects!", Monnam(mon));*/
		            pline("%sはよみがえった！", Monnam(mon));
			}
		        mon->mhp = mon->mhpmax;
		    ...
		    if(mon->data == &mons[PM_DEATH] && abstype == ZT_DEATH) {
		        if(cansee(mon->mx,mon->my)) {
			    hit(fltxt, mon, exclam(tmp));
/*JP		            pline("Death absorbs the deadly %s!",*/
		            pline("デスは死の%sを吸収した！",
				        type == ZT_BREATH(ZT_DEATH) ? 
/*JP				        "blast" : "ray");*/
				        "風" : "光線");
/*JP		            pline("It seems even stronger than before.");*/
...
/*JP			pline("%s is disintegrated!", Monnam(mon));*/
			pline("%sはこなごなになった！", Monnam(mon));
			mon->mgold = 0;
			otmp = mon->minvent;
			while(otmp) {
...
/*JP			You("don't feel hot!");*/
			...
			switch(rn2(5)) {
			case 0:
/*JP			    if (!rust_dmg(uarmh, "leather helmet", 0, FALSE))*/
			    if (!rust_dmg(uarmh, "皮の兜", 0, FALSE))
				continue;
			    break;
			case 1:
			    bodyhit = TRUE;
			    ...
			    break;
			case 2:
/*JP			    if (!rust_dmg(uarms, "wooden shield", 0, FALSE))*/
			    if (!rust_dmg(uarms, "木の盾", 0, FALSE))
				continue;
			    break;
			case 3:
/*JP			    if (!rust_dmg(uarmg, "gloves", 0, FALSE)) continue;*/
			    if (!rust_dmg(uarmg, "小手", 0, FALSE)) continue;
			    break;
			case 4:
/*JP			    if (!rust_dmg(uarmf, "boots", 0, FALSE)) continue;*/
			    if (!rust_dmg(uarmf, "靴", 0, FALSE)) continue;
			    break;
			}
			break; /* Out of while loop */
		    }
...
/*JP	    } else pline("%s whizzes by you!", The(fltxt));*/
	    } else pline("%sあなたのそばをかすめた！", The(fltxt));
...
/*JP	pay_for_damage(abstype == ZT_FIRE ?  "burn away" :
	...
	pay_for_damage(abstype == ZT_FIRE ?  "焼く" :
		       abstype == ZT_COLD ?  "粉砕する" :
...
/*JP		const char *msgtxt = "You hear hissing gas.";*/
		const char *msgtxt = "しゅーっと言うガスの音を聞いた．";
...
/*JP		    if (cansee(x,y)) msgtxt = "The water evaporates.";*/
		...
		if (lev->typ == WATER) {
		    ...
		    if (cansee(x,y))
/*JP			pline("The water freezes for a moment.");*/
			pline("水はちょっと凍った．");
		    else
/*JP			You("hear a soft crackling.");*/
			You("ピキピキと言う音を聞いた．");
		    rangemod -= 1000;	/* stop */
		} else {
		    rangemod -= 3;
	...
	if(closed_door(x, y)) {
		...
		case ZT_FIRE:
			...
			*shopdamage = TRUE;
		    ...
		    if(cansee(x,y)) {
/*JP			pline("The door is consumed in flames!");*/
			pline("扉は炎で焼きつくされた！");
			newsym(x,y);
		    }
/*JP		    else You("smell smoke.");*/
		    else You("煙の匂いがした．");
		    break;
		case ZT_COLD:
		    if(type >= 0 && *in_rooms(x, y, SHOPBASE)) {
			...
			*shopdamage = TRUE;
		    ...
		    unblock_point(x,y);	/* vision */
		    if(cansee(x,y)) {
/*JP			pline("The door freezes and shatters!");*/
			pline("扉は凍り，粉砕した！");
			newsym(x,y);
		    }
/*JP		    else You("feel cold.");*/
		    else You("冷気を感じた．");
		    break;
		case ZT_DEATH:
			...
			*shopdamage = TRUE;
		    ...
		    unblock_point(x,y);	/* vision */
		    if(cansee(x,y)) {
/*JP			pline("The door disintegrates!");*/
			pline("扉は粉砕した！");
			newsym(x,y);
		    }
		    else if(flags.soundok)
/*JP			You("hear crashing wood.");*/
			You("木の壊れる音を聞いた．");
		    break;
		case ZT_LIGHTNING:
		    if(type >= 0 && *in_rooms(x, y, SHOPBASE)) {
			...
			*shopdamage = TRUE;
		    ...
		    unblock_point(x,y);	/* vision */
		    if(cansee(x,y)) {
/*JP			pline("The door splinters!");*/
			pline("扉はずたずたになった！");
			newsym(x,y);
		    }
		    else if(flags.soundok)
/*JP			You("hear crackling.");*/
			You("ピキピキと言う音を聞いた．");
		    break;
		default:
		def_case:
		    if(cansee(x,y)) {
/*JP			pline("The door absorbs %s %s!",*/
			pline("扉は%s%sを吸収した！",
/*JP			      (type < 0) ? "the" : "your",
...
/*JP			    pline("%s glows a strange %s, but remains intact.",*/
				...
				The(xname(obj)),
/*JP				Hallucination ? hcolor() : "dark red");*/
				jconj_adj(Hallucination ? hcolor() : "暗赤色"));
		    }
		    quan = obj->quan;
		    switch(osym) {
			case POTION_CLASS:
