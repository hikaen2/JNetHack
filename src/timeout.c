...
{
	if (Slimed) {
/*JP	    pline_The("slime that covers you is burned away!");*/
	    pline("あなたを覆っていたべとべとは燃えてしまった！");
	    Slimed = 0L;
	}
	return;
}
...
nh_timeout()
{
	...
	if (u.usptime) {
		...
		find_ac();
		if (!Blind)
/*JP		    Norep("The %s haze around you %s.", hcolor(golden),
			  u.uspellprot ? "becomes less dense" : "disappears");
*/
		    Norep("あなたの回りの%s霧は%s．", hcolor(golden),
...
/*JP	    	pline("%s stops galloping.", Monnam(u.usteed));*/
...
/*JP				killer = "petrification";*/
		...
		case SLIMED:
			if (!killer) {
				killer_format = KILLED_BY;
/*JP				killer = "turning into green slime";*/
				killer = "緑スライムになった";
			} done(TURNED_SLIME);
			break;
		case VOMITING:
