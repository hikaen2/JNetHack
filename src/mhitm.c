...
/*JP			if (vis) pline("%s burns completely!", Monnam(mdef));*/
		    	if( vis) pline("%sは灰と化した！", Monnam(mdef));
			mondied(mdef);
			if (mdef->mhp > 0) return 0;
			else if (mdef->mtame && !vis)
/*JP			    pline("May %s roast in peace.", mon_nam(mdef));*/
			    pline("%sは丸焼けになったに違いない．", mon_nam(mdef));
...
/*JP			pline("%s slows down.", Monnam(mdef));*/
...
/*JP			pline("%s helmet blocks %s attack to %s head.",
