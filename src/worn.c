...
/*JP		Sprintf(buf, "%s", distant_name(old, doname));
		...
		Strcpy(m, jconj(jonmsg(best, &j), "た"));
		Strcpy(mm, jconj(joffmsg(best, &jj), "て"));

		Sprintf(buf, "%s", distant_name(old, doname));
		pline("%sは%s%s%s%s%s%s．", 
		    Monnam(mon), buf, jj,
		      Blind ? "はずして" : mm, 
...
/*JP		pline("%s puts on %s.", Monnam(mon),distant_name(best,doname));*/
		...
		pline("%sは%s%s%s．", Monnam(mon),
		      distant_name(best,doname), j,
		      Blind ? "身につける" : jconj(m, "た"));
	    }
...
*/
	...
	if (!can_saddle(mon)) {
		...
		m_lose_armor(mon, otmp);
		if (vis)
/*JP		    pline("%s saddle falls off.", s_suffix(Monnam(mon)));*/
		    pline("%sから鞍が落ちた．", s_suffix(Monnam(mon)));
	    }
	    if (mon == u.usteed)
		goto noride;
	} else if (mon == u.usteed && !can_ride(mon)) {
	noride:
/*JP	    You("can no longer ride %s.", mon_nam(mon));*/
	    You("もう%sに乗っていられない．", mon_nam(mon));
	    if (touch_petrifies(u.usteed->data) &&
			!Stone_resistance && rnl(3)) {
		char buf[BUFSZ];

/*JP		You("touch %s.", mon_nam(u.usteed));*/
		You("%sに触った．", mon_nam(u.usteed));
/*JP		Sprintf(buf, "falling off %s",
