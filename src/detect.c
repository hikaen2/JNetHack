...
/*JP	if (sobj) strange_feeling(sobj, "You feel materially poor.");*/
    ...
    if (stale) docrt();
/*JP    You("notice some gold between your %s.", makeplural(body_part(FOOT)));*/
...
outgoldmap:
    ...
    newsym(u.ux,u.uy);
/*JP    You("feel very greedy, and sense gold!");*/
...
/*JP	stuff = class ? oclass_names[class] : "objects";*/
	stuff = class ? jtrns_obj('S',oclass_names[class]) : "物体";
...
/*JP	pline("Too bad you can't see %s", the(bname));*/
	pline("悪いことに%sを見ることができない．", the(bname));
	return;
    }
...
/*JP	case 2 : pline("%s confuses you!", The(bname));*/
	case 2 : pline("%sを覗いてるとふらついたきた！", The(bname));
	    ...
	    break;
/*JP	case 3 : pline("%s damages your vision!", The(bname));*/
	case 3 : pline("%sを覗いていると視覚がおかしくなったきた！", The(bname));
...
/*JP	    losehp(rnd(30), "exploding crystal ball", KILLED_BY_AN);*/
    ...
    if (Hallucination) {
	if (!obj->spe) {
/*JP	    pline("All you see is funky %s haze.", hcolor());*/
	    pline("おお，ファンキーモンキーな%sもやが見える．", hcolor());
	} else {
	    switch(rnd(6)) {
/*JP	    case 1 : You("grok some groovy globs of incandescent lava.");*/
/*JP this message is original (by issei) */
...
/*JP	    case 2 : pline("Whoa!  Psychedelic colors, %s!",
		...
		break;
/*JP	    case 3 : pline("The crystal pulses with sinister %s light!", hcolor());*/
	    case 3 : pline("水晶は不吉な%sパルスを発した！", hcolor());
		break;
/*JP	    case 4 : You("see goldfish swimming above fluorescent rocks.");*/
	    case 4 : You("蛍光岩の上を金の魚が泳いでいるのを見た．");
		break;
...
/*JP    ch = yn_function("What do you look for?", NULL, '\0');*/
    ch = yn_function("何を探しますか？", NULL, '\0');
...
/*JP    You("peer into %s...", the(bname));*/
    ...
    nomovemsg = "";
    if (obj->spe <= 0)
/*JP	pline("The vision is unclear.");*/
	pline("映像は不鮮明になった．");
    else {
	int class;
	int ret = 0;
...
/*JP		You("see the Wizard of Yendor gazing out at you.");*/
		You("大魔法使いイェンダーがあなたを睨んでいるのを見た．");
/*JP	    else pline("The vision is unclear.");*/
...
/*JP				    You("feel an invisible monster!");*/
				    You("透明な怪物の気配を感じた！");
				else
...
/*JP				    You("find %s.", a_monnam(mtmp));*/
			...
			if ((trap = t_at(x,y)) && !trap->tseen && !rnl(8)) {
				...
				if ((otmp = sobj_at(STATUE, x, y)) != 0) {
				    ...
				    if (mtmp)
/*JP					You("find %s posing as a statue.",*/
					You("%sが像の振りをしているのを発見した．",
						  a_monnam(mtmp));
				    delobj(otmp);
				    exercise(A_WIS, TRUE);
				    if(Blind)
...
/*JP				You("find a%s.", traps[Hallucination ?*/
				You("%sを見つけた．", jtrns_obj('^',traps[Hallucination ?
