...
/*JP	    Your("%s %s!", rr, hcolor(warnings[warnlevel]));*/
	    Your("%sは%s輝いた！", rr, jconj_adj(hcolor(warnings[warnlevel])));
	}
}
...
/*JP	if(strcmp(string, "blast") && !thrown_weapon) {*/
	if((strcmp(string, "blast") && strcmp(string,"風") && strcmp(string,"息")) && !thrown_weapon) {
...
/*JP		if(!strcmp(string, "blast")) shieldeff(u.ux, u.uy);
		...
		if(!(strcmp(string, "blast")||strcmp(string,"風")))
		   shieldeff(u.ux, u.uy);
/*JP		pline_The("poison doesn't seem to affect you.");*/
		pline("毒は効かなかったようだ．");
		return;
	}
