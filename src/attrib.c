...
/*JP	t_abil[] = { {	10, &(Searching), "perceptive", "" },*/
	t_abil[] = { {	10, &(Searching), "知覚力を得た", "" },
/*JP		     {	20, &(HPoison_resistance), "hardy", "" },*/
		     {	20, &(HPoison_resistance), "免疫力を得た", "" },
		     {	 0, 0, 0, 0 } },
...
/*JP				You("feel %s!", abil->gainstr);*/
			...
			*(abil->ability) &= ~FROMEXPER;
			if((*(abil->ability) & INTRINSIC)) {
			    if(*(abil->losestr))
/*JP				You("feel %s!", abil->losestr);*/
				You("%sような気がした！", abil->losestr);
			    else if(*(abil->gainstr))
/*JP				You("feel less %s!", abil->gainstr);*/
				You("ちょっと%sような気がした！", abil->gainstr);
			}
		}
	    }
	}
