...
/*JP	if (usegold && !strcmp(word, "eat") && !metallivorous(youmonst.data))*/
	if (usegold && (!strcmp(word, "eat")||!strcmp(word,"食べる")) && !metallivorous(youmonst.data))
...
/*JP		else if (!strcmp(word, "wear") && *let==ARMOR_CLASS &&*/
		else if ((!strcmp(word, "wear") || !strcmp(word, "着る")) && *let==ARMOR_CLASS &&
...
/*JP	    case D_BROKEN:	dfeature = "broken door"; break;*/
	    case D_BROKEN:	dfeature = "壊れた扉"; break;
	    default:	cmap = S_vcdoor; break;	/* "closed door" */
	    }
	    /* override door description for open drawbridge */
	    if (is_drawbridge_wall(x, y) >= 0)
/*JP		dfeature = "open drawbridge portcullis",  cmap = -1;*/
...
/*JP	    Sprintf(altbuf, "altar to %s (%s)", a_gname(),*/
...
/*JP	    dfeature = "set of iron bars";*/
	    dfeature = "鉄の棒";

/*JP	if (cmap >= 0) dfeature = defsyms[cmap].explanation;*/
	if (cmap >= 0) 
...
/*JP			pline("But you can't reach it!");*/
	...
	if (dfeature)
/*JP		Sprintf(fbuf, "There is %s here.", an(dfeature));*/
...
/*JP		    You("%s no objects here.", verb);*/
	...
	if (skip_objects) {
	    if (dfeature) pline(fbuf);
/*JP	    pline("There are %s%s objects here.",
...
/*JP	    if (otmp->oinvis && !See_invisible) verb = "feel";*/
	    if (otmp->oinvis && !See_invisible)
		verb = "があるような気がした";
#endif
/*JP	    You("%s here %s.", verb, doname(otmp));*/
...
*/
	    if(poly_when_stoned(youmonst.data))
			You("%sの死体に素%sで触った．",
				jtrns_mon(mons[otmp->corpsenm].mname, -1), makeplural(body_part(HAND)));
	    else
/*JP			pline("Touching the %s corpse is a fatal mistake...",
				an(mons[corpse->corpsenm].mname));
*/
			pline("%sの死体に触れるのは致命的な間違いだ．．．",
 				jtrns_mon(mons[otmp->corpsenm].mname, -1));
		Sprintf(kbuf, "%sの死体で", jtrns_mon(mons[otmp->corpsenm].mname,-1));
		instapetrify(kbuf);
	}
}
...
doprinuse()
{
	...
	for (otmp = invent; otmp; otmp = otmp->nobj)
		...
		lets[ct++] = obj_to_let(otmp);
	lets[ct] = '\0';
/*JP	if (!ct) You("are not wearing or wielding anything.");*/
...
/*JP	len = strlen(class_name) + (unpaid ? sizeof "unpaid_" : sizeof "");*/
	len = strlen(class_name) + (unpaid ? sizeof "未払の_" : sizeof "");
	if (len > invbufsiz) {
	...
	if (unpaid)
/*JP	    Strcat(strcpy(invbuf, "Unpaid "), class_name);*/
