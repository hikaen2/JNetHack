...
/*JP	    if (!burn_dmg(item, "leather helmet")) continue;*/
	    if (!burn_dmg(uarmh, "革の兜")) continue;
	    break;
	case 1:
	    item = (victim == &youmonst) ? uarmc : which_armor(victim, W_ARMC);
	    if (item) {
/*JP		(void) burn_dmg(item, "cloak");*/
		(void) burn_dmg(item, "クローク");
		return TRUE;
	    }
...
/*JP		    pline("%s's %s %s not affected.", Monnam(victim), ostr,
			plural ? "are" : "is");*/
...
/*JP		    pline("%s's %s %s%s%s!", Monnam(victim), ostr, action[type],
			plural ? "" : "s",
			erosion+1 == MAX_ERODE ? " completely" :
			erosion ? " further" : "");*/
...
/*JP		    pline("%s's %s look%s completely %s.",
			Monnam(victim), ostr,
			plural ? "" : "s", msg[type]);*/
		    pline("%sの%sは完全に%s%s．", 
...
/*JP		pline("%s's %s %s %s", Monnam(victim),
		    ostr, plu ? "are" : "is", txt);*/
...
/*JP		pline("%s's %s %s", Monnam(victim), aobjnam(otmp,"are"), txt);*/
		pline("%sの%sは%s",Monnam(victim), xname(otmp), txt);
	}
	if (!rn2(2)) {
/*JP	    pline_The("grease dissolves.");*/
	    pline("油は溶けてしまった．");
	    otmp->greased = 0;
	}
}
...
/*JP	    You("find %s posing as a statue.", a_monnam(mon));*/
	    pline("%sが彫像のふりをしているのを見つけた．", a_monnam(mon));
...
/*JP		pline("Air currents pull you down!");*/
		pline("空気の流れがあなたを引き戻した");
...
/*JP			(void) rust_dmg(uarmh, "helmet", 1, TRUE, &youmonst);*/
...
/*JP			pline("%s your left %s!", A_gush_of_water_hits,*/
			pline("水が噴出してあなたの左%sに命中した！",
				    body_part(ARM));
/*JP			if (rust_dmg(uarms, "shield", 1, TRUE, &youmonst)) break;*/
			if (rust_dmg(uarms, "盾", 1, TRUE, &youmonst)) break;
			    break;
			if (uwep && bimanual(uwep))
			    goto two_hand;
			/* Two goto statements in a row--aaarrrgggh! */
/*JPglovecheck:		    (void) rust_dmg(uarmg, "gauntlets", 1, TRUE, &youmonst);*/
...
/*JP			pline("%s you!", A_gush_of_water_hits);*/
			...
			for (otmp=invent; otmp; otmp = otmp->nobj)
				    (void) snuff_lit(otmp);
			if (uarmc)
/*JP			    (void) rust_dmg(uarmc, "cloak", 1, TRUE, &youmonst);*/
			    (void) rust_dmg(uarmc, "クローク", 1, TRUE, &youmonst);
			else if (uarm)
/*JP			    (void) rust_dmg(uarm, "armor", 1, TRUE, &youmonst);*/
			    (void) rust_dmg(uarm, "鎧", 1, TRUE, &youmonst);
#ifdef TOURIST
			else if (uarmu)
/*JP			    (void) rust_dmg(uarmu, "shirt", 1, TRUE, &youmonst);*/
...
/*JP		You("%s onto a polymorph trap!",
		...
		You("変化の罠に%s！",
		    ...
		    locomotion(youmonst.data, "踏み込む"), "た"));
...
/*JP				pline("%s suddenly falls asleep!",*/
		...
		case RUST_TRAP:
			...
			case 0:
			    if (in_sight)
/*JP				pline("%s %s on the %s!", A_gush_of_water_hits,
				    mon_nam(mtmp), mbodypart(mtmp, HEAD));*/
			    pline("水が噴出して，%sの%sに命中した！", 
				  mon_nam(mtmp), mbodypart(mtmp, HEAD));

			    target = which_armor(mtmp, W_ARMH);
/*JP			    (void) rust_dmg(target, "helmet", 1, TRUE, mtmp);*/
			    (void) rust_dmg(target, "兜", 1, TRUE, mtmp);
			    break;
			case 1:
			    if (in_sight)
/*JP				pline("%s %s's left %s!", A_gush_of_water_hits,
				    mon_nam(mtmp), mbodypart(mtmp, ARM));
*/
				pline("水が噴出して，%sの左の%sに命中した！",
				      mon_nam(mtmp), mbodypart(mtmp, ARM));
			    target = which_armor(mtmp, W_ARMS);
/*JP			    if (rust_dmg(target, "shield", 1, TRUE, mtmp))
				break;
*/
...
/*JP			    (void) rust_dmg(target, "gauntlets", 1, TRUE, mtmp);*/
			    (void) rust_dmg(target, "小手", 1, TRUE, mtmp);
			    break;
			case 2:
			    if (in_sight)
/*JP				pline("%s %s's right %s!", A_gush_of_water_hits,
				    mon_nam(mtmp), mbodypart(mtmp, ARM));
*/
				pline("水が噴出して，%sの右の%sに命中した！",
				    mon_nam(mtmp), mbodypart(mtmp, ARM));
...
*/
			    ...
			    if (target)
/*JP				(void) rust_dmg(target, "cloak", 1, TRUE, mtmp);*/
				(void) rust_dmg(target, "クローク", 1, TRUE, mtmp);
			    else {
				target = which_armor(mtmp, W_ARM);
				if (target)
/*JP				    (void) rust_dmg(target, "armor", 1, TRUE, mtmp);*/
				    (void) rust_dmg(target, "鎧", 1, TRUE, mtmp);

#ifdef TOURIST
				else {
				    target = which_armor(mtmp, W_ARMU);
/*JP				    (void) rust_dmg(target, "shirt", 1, TRUE, mtmp);*/
...
/*JP			    fallverb = "is dragged";	 * sokoban pit */
			    fallverb = "ずり落ちる";	/* sokoban pit */
			}
			if (!passes_walls(mptr))
			    mtmp->mtrapped = 1;
			if (in_sight) {
/*JP			    pline("%s %s into %s pit!",
...
/*JP			mselftouch(mtmp, "Falling, ", FALSE);*/
		...
		case TRAPDOOR:
			...
			if (is_flyer(mptr) || is_floater(mptr) ||
				...
				if (in_sight) {
/*JP				    pline("%s seems to be yanked down!",
					  Monnam(mtmp));
*/
				    pline("%sは引き落されたようだ！",
					  Monnam(mtmp));
...
const char *arg;
{
	...
	if(uwep && uwep->otyp == CORPSE && touch_petrifies(&mons[uwep->corpsenm])
			&& !Stone_resistance) {
/*JP		pline("%s touch the %s corpse.", arg,
		        mons[uwep->corpsenm].mname);
		Sprintf(kbuf, "%s corpse", an(mons[uwep->corpsenm].mname));
*/
		pline("%s%sの死体に触った．", arg,
		        jtrns_mon(mons[uwep->corpsenm].mname,-1));
		Sprintf(kbuf, "%sの死体で", jtrns_mon(mons[uwep->corpsenm].mname,-1));
		instapetrify(kbuf);
	}
...
boolean byplayer;
{
	...
	if (mwep && mwep->otyp == CORPSE && touch_petrifies(&mons[mwep->corpsenm])) {
		if (cansee(mon->mx, mon->my)) {
/*JP			pline("%s%s touches the %s corpse.",
			    arg ? arg : "", arg ? mon_nam(mon) : Monnam(mon),
			    mons[mwep->corpsenm].mname);
*/
			pline("%s%sは%sの死体に触った．", 
			    arg ? arg : "", arg ? mon_nam(mon) : Monnam(mon),
			    jtrns_mon(mons[mwep->corpsenm].mname,-1));
		}
		minstapetrify(mon, byplayer);
	}
}
...
/*JP		pline("It feels as though you've lost some weight.");*/
		You("まるで体重が減ったように感じた．");
	else if(u.uinwater)
		spoteffects();
	else if(u.uswallow)
...
/*JP		You("start to float in the air!");*/
	...
	if (u.usteed && !is_floater(u.usteed->data) && !is_flyer(u.usteed->data)){
	    if (Lev_at_will)
/*JP	    	pline("%s magically floats up!", Monnam(u.usteed));*/
	    	pline("%sは魔法の力で浮いた！", Monnam(u.usteed));
	    else {
/*JP	    	You("cannot stay on %s.", mon_nam(u.usteed));*/
...
/*JP	    losehp(num, tower_of_flame, KILLED_BY_AN);*/
...
/*JP			You("aren't skilled enough to reach from %s.",
				mon_nam(u.usteed));
*/
...
/*JP		You("grab the trapped %s using your bare %s.",
				mtmp->data->mname, makeplural(body_part(HAND)));
*/
		You("罠にかかっている%sを素%sで掴んだ．",
				jtrns_mon(mtmp->data->mname, mtmp->female), makeplural(body_part(HAND)));
...
/*JP			Sprintf(kbuf, "trying to help %s out of a pit",
					an(mtmp->data->mname));
*/
...
/*JP			} else pline("That %s was not trapped.", xname(otmp));*/
