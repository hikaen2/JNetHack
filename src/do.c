...
/*JP		pline_The("ring is regurgitated!");*/
		pline("指輪は逆流した！");
giveback:
		obj->in_use = FALSE;
		dropx(obj);
...
/*JP		pline_The("water flow seems fixed.");*/
		pline("水の流れが一定になった．");
		break;
...
/*JP		pline_The("water flow %s the drain.",
			(obj->spe<0) ? "misses" : "hits");
*/
		pline_The("水が排水口%s．",
			(obj->spe<0) ? "から反れるようになった" : "めがけて流れるようになった");
		break;
	    case RIN_INCREASE_DAMAGE:
/*JP		pline_The("water's force seems %ser now.",
			(obj->spe<0) ? "small" : "great");
*/
		pline_The("水の勢いが%sなった．",
			(obj->spe<0) ? "小さく" : "強く");
		break;
	    case RIN_HUNGER:
		ideed = FALSE;
...
/*JP		pline("Several flies buzz around the sink.");*/
		pline("数匹のハエがブンブン流し台の回りを飛びまわった．");
		break;
	    default:
		ideed = FALSE;
...
/*JP			pline ("The leash is tied around your %s.",*/
	...
	if (obj->owornmask & W_SADDLE) {
		if (*word)
/*JP			You("cannot %s %s you are sitting on.", word,
				something);
*/
...
/*JP		You("are held back by your pet!");*/
	...
	if (trap)
/*JP	    You("%s %s.", locomotion(youmonst.data, "jump"),
