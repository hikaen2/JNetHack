...
/*JP	      pline("%s tries to rob you, but there is nothing to steal!",*/
...
*/
			    pline("%sはあなたを魅了した．あなたはよろこんで%sを%s%s．",
				  ...
				  equipname(otmp),
 				  (objects[otmp->otyp].oc_delay > 1) ? "はずし始め" : "はずして",
				  curssv ? "あげた" : "手渡した");
			else
...
*/
			    pline("%sはあなたを誘惑した．あなたは%sを%s．",
				  Blind ? "何者か" : Monnam(mtmp),
				  equipname(otmp),
 				  (objects[otmp->otyp].oc_delay > 1) ? "はずし始めた" : "はずした");
			named++;
